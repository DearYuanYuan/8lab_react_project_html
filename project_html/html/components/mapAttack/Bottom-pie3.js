...
export default class echartsPie extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        var data;
        if (JSON.stringify(nextProps.BottomPieDos) != JSON.stringify(this.props.BottomPieDos)) {
            this.setState({
                BottomPieHttp:nextProps.BottomPieDos
            })
            data={
                "name":'',
                "value":nextProps.BottomPieDos
            };
        }else{
            this.setState({
                BottomPieDos:this.props.BottomPieDos
            })
            data={
                "name":'',
                "value":this.props.BottomPieDos
            };
        }
        ...
        var chart = echarts.init(document.getElementById('Bottom-pie3'));
        ...
        var chartLine = echarts.init(document.getElementById('Bottom-Line3'));
    ...
    render() {
        return (
            <div  className='Bottom-PieStyle'>
                <div id="Bottom-pie3" style={{ width: "80px", height: "80px", position: 'absolute', right: '0', top: '0' }}></div>
                <div style={{ width: "65px", height: "42px", position: 'absolute', top: '46px', background: 'linear-gradient(-1deg,  #2A2D36 30%, #2E323C 70%)' }}>
                    <div style={{ fontSize: '12px', color: '#A1A6AD', textAlign: 'right' }}>dos攻击</div>
                </div>
                <div id="Bottom-Line3" style={{ width: "100%", height: "100%", position: 'absolute', left: '0', bottom: '-38px', }}></div>
