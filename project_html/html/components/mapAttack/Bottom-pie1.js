...
export default class echartsPie extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        var data;
        if (JSON.stringify(nextProps.BottomPieHttp) != JSON.stringify(this.props.BottomPieHttp)) {
            this.setState({
                BottomPieHttp:nextProps.BottomPieHttp
            })
            data={
                "name":'',
                "value":nextProps.BottomPieHttp
            };
        }else{
            this.setState({
                BottomPieHttp:this.props.BottomPieHttp
            })
            data={
                "name":'',
                "value":this.props.BottomPieHttp
            };
        }
        ...
        var chart = echarts.init(document.getElementById('Bottom-pie1'));
        ...
        var chartLine = echarts.init(document.getElementById('Bottom-Line1'));
    ...
    render() {
        return (
            <div className='Bottom-PieStyle' >
                <div id="Bottom-pie1" style={{ width: "80px", height: "80px", position: 'absolute', right: '0px', top: '0px' }}></div>
                <div style={{ width: "65px", height: "42px", position: 'absolute', top: '46px', background: 'linear-gradient(-1deg,  #2A2D36 30%, #2E323C 70%)' }}>
                    <div style={{ fontSize: '12px', color: '#A1A6AD', textAlign: 'right' }}>http防御</div>
                </div>
                <div id="Bottom-Line1" style={{ width: "100%", height: "100%", position: 'absolute', left: '0px', bottom: '-38px', }}></div>
