...
export default class echartsPie extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        var data;
        if (JSON.stringify(nextProps.BottomPieWeb) != JSON.stringify(this.props.BottomPieWeb)) {
            this.setState({
                BottomPieHttp:nextProps.BottomPieWeb
            })
            data={
                "name":'',
                "value":nextProps.BottomPieWeb
            };
        }else{
            this.setState({
                BottomPieWeb:this.props.BottomPieWeb
            })
            data={
                "name":'',
                "value":this.props.BottomPieWeb
            };
        }
        ...
        var chart = echarts.init(document.getElementById('Bottom-pie2'));
        ...
        var chartLine = echarts.init(document.getElementById('Bottom-Line2'));
    ...
    render() {
        return (
            <div  className='Bottom-PieStyle' >
                <div id="Bottom-pie2" style={{ width: "80px", height: "80px", position: 'absolute', right: '0', top: '0' }}></div>
                <div style={{ width: "65px", height: "42px", position: 'absolute', top: '46px', background: 'linear-gradient(-1deg,  #2A2D36 30%, #2E323C 70%)' }}>
                    <div style={{ fontSize: '12px', color: '#A1A6AD', textAlign: 'right' }}>web攻击</div>
                </div>
                <div id="Bottom-Line2" style={{ width: "100%", height: "100%", position: 'absolute', left: '0', bottom: '-38px', }}></div>
