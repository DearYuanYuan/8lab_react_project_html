...
export default class VersionMsg extends React.Component {
    ...
    * progressId ： 任务id
    * */
    loopGetProgress(progressId){
        var self = this
        this.progressInterval =setInterval(function () {
            self.getProgressTime(progressId)
    ...
    * */
    handleVersionConformBack() {
        ...
        $.ajax({
            ...
            success: function (data) {
                // console.log(data[0].result.task_id)
                //获取进度条
                                ...
                                this.state.versionPageMsg && this.state.versionPageMsg.map(function (data, index) {
                                    return (
                                        <div className="dataList-content" key={index}>
                                            <table className="databaseHostList" onClick={this.handleToggleMsg.bind(this, index)}>
                                                {
                                                    index == 0 &&  //列表第一项展示表头
                                                    <thead>
                                                        <tr>
                                            ...
                                            <div className="showDatabaseHostList versionMsg">
                                                <h3 className="versionMsgTitle">备注信息：更新项目的README文件。</h3>
                                                {/*
                                                * "token": "trust2",//暂无
                                                  "username": data.operate_username,
                                                  "host_name": data.protect_host_name,
                                                  ...
                                                  "org_version_tx_id": data.version_txid
                                                * */}
                                                <button className="backToVersion plugAction"
